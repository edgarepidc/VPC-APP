...
const pillClass: Record<DeliverableStatus, string> = {
  ...
  delivered: "bg-[#EEEDFE] text-[#3C3489]",
};

const statusBtnActive: Record<DeliverableStatus, string> = {
  pending: "bg-[#F1EFE8] text-[#444441] border-[#B4B2A9]",
  ...
  delivered: "bg-[#EEEDFE] text-[#3C3489] border-[#AFA9EC]",
};
...
function dayChip(due: string | null, status: string) {
  ...
  if (df < 0)
    return (
      <span className="ml-1 inline-block rounded-full bg-[#FCEBEB] px-1.5 py-0.5 text-[10px] font-medium text-[#791F1F]">
        {Math.abs(df)}d vencido
      </span>
    );
  if (df === 0)
    return (
      <span className="ml-1 inline-block rounded-full bg-[#FAEEDA] px-1.5 py-0.5 text-[10px] font-medium text-[#633806]">
        hoy
      </span>
    );
  if (df <= 5)
  ...
  return (
    <span className="ml-1 inline-block rounded-full bg-[#EAF3DE] px-1.5 py-0.5 text-[10px] font-medium text-[#27500A]">
      {df}d
    </span>
  );
}
...
function acuseMini(acuses: DeliverableAcuse[]) {
  if (acuses.length === 0)
    return <span className="text-[11px] text-[#9a9893]">Sin acuses</span>;
  ...
  return (
    <span className="text-[11px]">
      ...
      <span className="text-[#9a9893]"> confirmados</span>
    </span>
  );
}
...
export function DeliverablesTracker({ rows, projects, canEdit }: Props) {
  ...
  return (
    <div
      className="flex min-h-[calc(100vh-8rem)] flex-col rounded-xl border border-black/[0.09] bg-[#f0ede8] text-[#1a1a18]"
      style={
        {
    ...
    >
      <header className="sticky top-0 z-[100] flex flex-wrap items-center justify-between gap-2.5 border-b border-black/[0.09] bg-white px-5 py-3.5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-[15px] font-semibold">Tracker de Entregables</div>
      ...
      <div className="flex flex-1 flex-col overflow-hidden lg:flex-row">
        <div className="min-w-0 flex-1 space-y-3.5 overflow-y-auto p-[18px] pb-10">
          <div className="grid grid-cols-[repeat(auto-fit,minmax(110px,1fr))] gap-2">
            <div className="rounded-xl border border-black/[0.09] bg-white px-3.5 py-2.5">
              <div className="mb-0.5 text-[11px] text-[#5a5955]">Total entregables</div>
              <div className="text-[22px] font-semibold leading-none">{total}</div>
              <div className="mt-0.5 text-[10px] text-[#9a9893]">registrados</div>
            </div>
            <div className="rounded-xl border border-black/[0.09] bg-white px-3.5 py-2.5">
              <div className="mb-0.5 text-[11px] text-[#5a5955]">Completados</div>
              <div className="text-[22px] font-semibold leading-none text-[#27500A]">{comp}</div>
              <div className="mt-0.5 text-[10px] text-[#9a9893]">aprobados / entregados</div>
            </div>
            <div className="rounded-xl border border-black/[0.09] bg-white px-3.5 py-2.5">
              <div className="mb-0.5 text-[11px] text-[#5a5955]">En proceso</div>
              <div className="text-[22px] font-semibold leading-none">{pend}</div>
              <div className="mt-0.5 text-[10px] text-[#9a9893]">pendiente o revisión</div>
            </div>
            <div className="rounded-xl border border-black/[0.09] bg-white px-3.5 py-2.5">
              <div className="mb-0.5 text-[11px] text-[#5a5955]">Vencidos</div>
              <div
              ...
              <div className="mt-0.5 text-[10px] text-[#9a9893]">fuera de fecha</div>
            </div>
            <div className="rounded-xl border border-black/[0.09] bg-white px-3.5 py-2.5">
              <div className="mb-0.5 text-[11px] text-[#5a5955]">Acuses pendientes</div>
              <div
                className="text-[22px] font-semibold leading-none"
              ...
              <div className="mt-0.5 text-[10px] text-[#9a9893]">sin confirmar</div>
            </div>
            <div
              className="rounded-xl border-2 px-3.5 py-2.5"
              style={{ borderColor: ACCENT_L, background: "white" }}
            >
              <div className="mb-0.5 text-[11px] text-[#5a5955]">Avance ponderado</div>
              <div className="text-[22px] font-semibold leading-none" style={{ color: ACCENT }}>
                {pct}%
              </div>
              <div className="mt-0.5 text-[10px] text-[#9a9893]">
                {donePeso} de {totalPeso} pts completados
              </div>
            </div>
          </div>

          <div className="rounded-xl border border-black/[0.09] bg-white px-[18px] py-3.5">
            <div className="mb-2 flex flex-wrap items-baseline justify-between gap-2">
              <div>
                ...
                <div className="mt-0.5 text-[11px] text-[#5a5955]">
                  El porcentaje refleja el <strong className="text-[#1a1a18]">peso relativo</strong>{" "}
            ...
            <div className="flex flex-wrap gap-3.5 text-[11px] text-[#5a5955]">
              ...
              <span className="ml-auto text-[11px] text-[#9a9893]">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <input
              ...
              className="h-[34px] w-[200px] max-w-full rounded-lg border border-black/[0.17] bg-white px-2.5 text-[13px] text-[#1a1a18] outline-none focus:border-[var(--accent)] focus:ring-2 focus:ring-[#EEEDFE]/50"
            />
            <select
              value={st}
              onChange={(e) => setSt(e.target.value)}
              className="h-[34px] rounded-lg border border-black/[0.17] bg-white px-2.5 text-[13px] outline-none focus:border-[var(--accent)]"
            >
          ...
          <div className="overflow-hidden rounded-xl border border-black/[0.09] bg-white">
            <div className="overflow-x-auto">
              <table className="w-full min-w-[900px] border-collapse text-xs">
                <thead>
                  <tr className="border-b border-black/[0.09] bg-[#f7f5f1] text-left text-[11px] font-medium text-[#5a5955]">
                ...
                <tbody>
                  {filtered.map((d) => {
                    ...
                    return (
                      <tr
                        ...
                        className={`cursor-pointer border-b border-black/[0.09] last:border-b-0 hover:bg-[#f7f5f1] ${
                          selectedId === d.id && panel === "detail"
                            ? "bg-[#EEEDFE] hover:bg-[#EEEDFE]"
                            : ""
                        } ${pending ? "opacity-60" : ""}`}
                      >
                        <td className="px-3 py-2.5 font-mono text-[10px] text-[#9a9893]">
                          {d.displayId}
                        </td>
                        <td className="px-3 py-2.5 text-[11px] text-[#5a5955]">{d.projectName}</td>
                        <td className="px-3 py-2.5 text-sm font-medium leading-snug">{d.title}</td>
                        <td className="px-3 py-2.5 text-[11px] text-[#5a5955]">{d.phase || "—"}</td>
                        <td className="px-3 py-2.5 text-[11px] text-[#5a5955]">
                          {d.ownerName || "—"}
                        </td>
                        ...
                        <td className="px-3 py-2.5">
                          <span
                            className={`inline-block rounded-full px-2 py-0.5 text-[10px] font-medium ${pillClass[stKey]}`}
                          >
                        ...
                        <td className="px-3 py-2.5">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-[11px] font-medium text-[#5a5955]">
                  ...
                  {filtered.length === 0 ? (
                    <tr>
                      <td colSpan={9} className="px-3 py-8 text-center text-[13px] text-[#9a9893]">
        ...
        <aside
          className={`shrink-0 overflow-y-auto border-black/[0.09] bg-white transition-[width] duration-200 ease-out lg:border-l ${
...
function DetailPanel({
  ...
  return (
    <div className="w-full min-w-[min(100vw,430px)] max-w-full p-[18px] lg:w-[430px]">
      <div className="mb-4 flex items-start justify-between gap-2 border-b border-black/[0.09] pb-3">
        <div>
          <div className="text-[11px] text-[#5a5955]">
          ...
          <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
            <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${pillClass[stKey]}`}>
              {DELIVERABLE_STATUS_LABEL[stKey]}
            </span>
      ...
      {canEdit ? (
        <>
          <div className="mb-3.5 text-[10px] font-semibold uppercase tracking-wide text-[#5a5955]">
            Cambiar estado
          </div>
          <div className="mb-3.5 flex flex-wrap gap-1">
            {DELIVERABLE_STATUSES.map((s) => (
              <button
                ...
                className={`rounded-full border border-black/[0.17] px-3 py-1 text-[11px] font-medium text-[#5a5955] hover:bg-[#f7f5f1] ${
      ...
      <div className="mb-3.5 rounded-lg border border-black/[0.09] p-3">
        <div className="mb-2.5 text-[11px] font-semibold">Acuse del cliente / stakeholders</div>
        {row.acuses.length === 0 ? (
          <div className="py-1 text-xs text-[#9a9893]">Sin acuses registrados</div>
        ) : (
          row.acuses.map((ac, i) => (
            <div
              key={`${ac.name}-${i}`}
              className="flex items-start gap-2 border-b border-black/[0.09] py-1.5 last:border-b-0"
            >
              <button
                type="button"
                disabled={!canEdit}
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-[11px] text-[#5a5955]">{ac.role}</div>
              ...
              {canEdit ? (
                <button
                  ...
                  className="shrink-0 rounded border border-black/[0.09] px-1.5 py-0.5 text-[11px] text-[#9a9893] hover:bg-[#f7f5f1]"
                >
                  ✕
                </button>
              ) : null}
        ...
        {canEdit ? (
          <div className="mt-2 flex flex-wrap gap-1.5">
            ...
            <button
              ...
              className="h-[30px] rounded-lg border border-black/[0.17] bg-white px-2.5 text-[11px] font-medium hover:bg-[#f7f5f1]"
            >
              + Agregar
            </button>
          </div>
      ...
      <div className="mb-2 text-[10px] font-semibold uppercase tracking-wide text-[#5a5955]">
        Historial de actividad
      </div>
      <div className="mb-4 space-y-0">
        {row.activityLog.length === 0 ? (
          <p className="text-xs text-[#9a9893]">Sin movimientos.</p>
        ) : (
          [...row.activityLog].reverse().map((h, i) => (
            <div
              key={`${h.at}-${i}`}
              className="flex gap-2 border-b border-black/[0.09] py-1.5 last:border-b-0"
            >
              <span
                className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full"
                style={{ background: h.color ?? "#888780" }}
              />
              <span className="w-14 shrink-0 whitespace-nowrap text-[10px] text-[#9a9893]">
                {formatLogAt(h.at)}
              </span>
              <span className="text-xs leading-snug text-[#5a5955]">{h.text}</span>
            </div>
          ))
        )}
      </div>
...
function ReadOnlyDetail({ row, myPct }: { row: DeliverableTrackerRow; myPct: number }) {
  return (
    <>
      ...
      >
        <div className="min-w-0 flex-1">
          <div className="text-[11px] font-semibold" style={{ color: ACCENT_D }}>
            Peso / esfuerzo
          </div>
          <div className="text-[10px] opacity-80" style={{ color: ACCENT_D }}>
...
function Field({ label, value }: { label: string; value: string | null }) {
  return (
    <div className="mb-3">
      <div className="mb-1 text-[10px] font-semibold uppercase tracking-wide text-[#5a5955]">
        {label}
      </div>
      <div className="text-[13px] leading-relaxed text-[#1a1a18]">{value?.trim() || "—"}</div>
    </div>
  );
}
    ...
    <>
      ...
      <div className="mb-3 grid grid-cols-1 gap-2.5 sm:grid-cols-2">
        <div className="sm:col-span-2">
          <label className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-[#5a5955]">
            Proyecto
          </label>
          <select
            value={projectId}
...
function CreatePanel({
  ...
  return (
    <div className="w-full min-w-[min(100vw,430px)] max-w-full p-[18px] lg:w-[430px]">
      <div className="mb-4 flex items-start justify-between gap-2 border-b border-black/[0.09] pb-3">
        <div>
          <div className="text-[11px] text-[#5a5955]">Gestión del alcance · PMBOK</div>
      ...
      <div className="mb-3.5 rounded-lg border border-black/[0.09] bg-[#f7f5f1] p-2.5 text-[11px] leading-relaxed text-[#5a5955]">
        El <strong className="text-[#1a1a18]">peso / esfuerzo</strong> define cuánto contribuye este
